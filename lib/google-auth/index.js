...
var googleAuth = function(params) {
  ...
  if (params.app) {
      ...
      if (query.code) {
        ...
        self.oauth2Client.getToken(query.code, function(err, tokens) {
          ...
          if (req.session.action) {
            console.log("***** sdession has action, which we are redirecring to " + req.session.action);

            res.redirect(req.session.action);
          }
          else {
            console.log("***** sdession has no action, redirecting to /");
            return res.redirect('/');
          }
        });
    ...
    // Logout.
    params.app.get('/logout', function (req, res) {
